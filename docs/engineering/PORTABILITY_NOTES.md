...
Изначально определение `cwd` в `crates/process` было реализовано только для Linux.

Сейчас уже есть два подтвержденных пути:

- чтение `/proc/<pid>/cwd`.
- чтение через `proc_pidinfo(..., PROC_PIDVNODEPATHINFO, ...)` на macOS.

Для остальных платформ код по-прежнему возвращает аккуратную ошибку о неподдерживаемом способе определения `cwd`, но полноценной альтернативной реализации пока нет.
...
На этом этапе провалы тестов на macOS показывали другую грань той же проблемы: код уже компилировался, но многие операции в `app` и `tabs` неявно требовали, чтобы `ShellProcess::current_dir()` умел работать на текущей платформе.

Тогда картина была такой:

- на Linux `current_dir()` использует `/proc/<pid>/cwd`
- на остальных платформах путь для `cwd` отсутствовал
...
На macOS эта неподдерживаемая ветка начинала ломать не только прямые тесты на `cwd`, но и широкий набор внешне несвязанных тестов. Причина в том, что snapshot tab/pane состояния всегда пытается прочитать живой `cwd` каждого процесса:
...
- в `crates/process` появился macOS-путь определения `cwd`;
- в `crates/tabs` снимок состояния использует живой `cwd` или последний известный каталог панели.

Но с точки зрения переносимости итог все равно остается ограниченным:

- модель сохранения по-прежнему зависит от live introspection процесса, просто теперь эта зависимость уже закрыта и для macOS;
- за пределами Linux и macOS полного платформенного слоя все еще нет;
- переносимость в этой области заметно лучше, чем была в момент первых провалов, но не завершена архитектурно.

Иными словами, главный macOS-блокер по `cwd` уже снят, но общая задача переносимости `current_dir()` на произвольные платформы остается открытой.
...
Позже этот путь был немного упорядочен:

- общий код теперь отделяет доставку прерывания от платформенного шага post-interrupt recovery;
- Linux-специфичная доводка после `SIGINT` вынесена в отдельный платформенный хук;
- macOS использует более короткий путь без Linux-only `/proc`-очистки.

Но это улучшило в первую очередь границу кода, а не сняло все переносимые ограничения.
